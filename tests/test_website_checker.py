...
def test_collect_metrics_200():
    r = collect_metrics('https://httpstat.us/200', publish=False)
...
def test_collect_metrics_500():
    r = collect_metrics('https://httpstat.us/500', publish=False)
...
def test_collect_metrics_wiki_good_regex():
    r = collect_metrics('https://www.wikipedia.org/', pattern="English", publish=False)
...
def test_collect_metrics_wiki_bad_regex():
    r = collect_metrics('https://www.wikipedia.org/', pattern="OIWQWFHBVWFJODSDF", publish=False)
    assert r.get('url') == 'https://www.wikipedia.org/'
    assert r.get('status_code') == 200
    assert r.get('response_time') > 0
